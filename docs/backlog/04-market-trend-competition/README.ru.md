...
Status: **DONE** — QA-гейт пройден (2026-07-19). Depends on: 01 · Operator-requested (Jul 19)

> **Артефакты:** [design.md](design.md) rev.3 (3 раунда спек-ревью) · [plan.md](plan.md) rev.2 ·
> [tracker.md](tracker.md) · [qa-report-04.md](qa-report-04.md) · [handoff.md](handoff.md) —
> **читать handoff.md перед тем, как потреблять выход 04 в фичах 05/06/09/10.**
>
> **Построено:** `lib/f04/{config,scoring,provenance}.js` (141 юнит-тест) · три n8n-воркфлоу —
> `f04-market-intel` (111 узлов), `f04-competition-intel` (64), `f04-db-write` (7) ·
> три набора артефактов AI-агентов в [agents/](agents/), схемы проверены на живом API OpenAI
> в strict-режиме.
>
> **Проверено сквозным прогоном** на реальной pre-seed компании из Show HN (заявка из радара,
> без дека): найдены MEDITECH / Oracle Health / Epic / Microsoft Nuance DAX — ни одного из них
> фаундер не называл, а это и есть главный вывод фичи — и записаны честные `missing`-claims по
> TAM, росту и why-now вместо выдуманных значений. `market.outlook` показал `undetermined`,
> а не уверенное `neutral`, на неисследованной категории. **Ноль миграций схемы** — конкуренты
> живут как `claims` согласно 01 design §9.
...
- Thesis-template «два блока: механизм ценности + тайминг why-now» (screening →
  структура memo).
