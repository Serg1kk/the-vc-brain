...
// ============================================================================
// Negation over-triggering -- QA pass Finding 2(b), TWO rounds. Round 1: a
// bare "absent from source" rule fired on ordinary, true, self-aware
// pre-seed disclosures ("does not currently generate revenue") the instant
// an unrelated, independently-collected source simply didn't happen to
// restate the same negation -- the normal case. Fixed to require the source
// POSITIVELY asserting the opposite before flagging. Round 2: the round-1
// fix's loose multi-word predicate sweep let an INCIDENTAL nearby word (not
// the actual negated predicate) match an unrelated positive mention
// elsewhere in the source. Fixed to derive exactly ONE precise predicate.
// ============================================================================
...
describe('quoteSalienceMismatches -- negation over-triggering (QA Finding 2b)', () => {
  ...
  test('round 2: an incidental nearby word must NOT be mistaken for the negated predicate', () => {
    // The genuinely negated predicate here is "charge" (correctly absent
    // from the source). The old multi-word sweep also picked up the
    // trailing "beta", which incidentally matches the source's own
    // unrelated "closed beta" -- two true, mutually consistent statements,
    // wrongly flagged as contradictory.
    const quote = 'We have no paying customers yet and do not charge for the beta.';
    const source = 'The product is currently in closed beta with five design-partner teams providing weekly feedback before a public launch.';
    assert.deepEqual(quoteSalienceMismatches(quote, source), []);
  });
  ...
  test('regression: a genuine liability flip still fires (source positively asserts the opposite)', () => {
    ...
    // the false-positive fixes above, not just inherited from the older test.
    const quote = 'The company shall not indemnify the buyer for third-party claims.';
    const source = 'Per the agreement, the company shall indemnify the buyer for third-party claims arising from the deal.';
    const mismatches = quoteSalienceMismatches(quote, source);
    assert.ok(
