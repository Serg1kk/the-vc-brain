...
// ============================================================================
// Row bookkeeping -- group a flat row array by founder_id once per
// attribute, rather than re-scanning on every candidate (122 founders is
// trivially fast either way, but this keeps the per-candidate loop O(1)
// lookups instead of O(n) filters).
// ============================================================================

function groupByFounder(rows) {
  const map = new Map();
  for (const row of Array.isArray(rows) ? rows : []) {
    if (!row || row.founder_id == null) continue;
    if (!map.has(row.founder_id)) map.set(row.founder_id, []);
    map.get(row.founder_id).push(row);
  }
  return map;
}
...
function gatherCandidateIds(positiveAttrs, fetchedRows) {
  const idSet = new Set();
  for (const attr of positiveAttrs) {
    const rows = Array.isArray(fetchedRows[attr.id]) ? fetchedRows[attr.id] : [];
    for (const row of rows) {
      if (row && row.founder_id != null) idSet.add(row.founder_id);
    }
  }
  ...
  if (resolved.state === STATES.MATCHED || resolved.state === STATES.MATCHED_BROADENED) {
    if (attr.polarity === 'negative') {
      ...
      view.evidence = {
        ...
        quote_verbatim: entry.quote_verbatim != null ? entry.quote_verbatim : (row.text_verbatim != null ? row.text_verbatim : null),
...
// ============================================================================
// §5.4 rule 6, rev.5 -- total ordering: `bucket_ordinal DESC, rank_score
// DESC NULLS LAST, founder_id ASC`. `confidence` is DELIBERATELY no longer
// a sort key (rev.4's `rank_score DESC, confidence DESC, founder_id ASC` is
...
//   compareByBucket   -- the default. Sorts the ORDINAL INTEGER (never the
//                         bucket string -- alphabetically 'high' < 'low' <
//                         'mid', so a naive string DESC sort silently
//                         inverts to mid -> low -> high).
...
function score(plan, fetchedRows) {
  ...
  for (const attr of attributes) {
    rowsByAttrFounder.set(attr.id, groupByFounder(rows[attr.id]));
  }

  const { ids: candidateIds, truncated } = gatherCandidateIds(positiveAttrs, rows);
