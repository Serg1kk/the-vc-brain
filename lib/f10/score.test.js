...
// test, plus spec-delta / QA-gate additions that arrived after this file
...
//   QA gate, finding A -- `quote_verbatim` must never fall back to
//            `claims.text_verbatim`; a missing real quote must render as
//            `null`, never a fabricated citation.
//   QA gate, finding B -- structural (company-scoped) claim rows carry no
//            `founder_id` and must resolve through the candidate founder's
//            `company_id`/`application_id` instead, or they can never match.
//
...
// ============================================================================
// QA gate, finding A -- `quote_verbatim` must never fall back to
// `claims.text_verbatim`. Live: `evidence` rows with relation='supports'
// are 590 total, 398 with a real quote -- the other 32.5% were rendering
// as a fabricated citation under this bug (REQ-004 / Agentic Traceability
// inverted). Fix: `quote_verbatim` is `null` unless a REAL quote exists on
// the evidence entry; `claim_text` carries the claim's own text separately,
// always; `quote_source` names which one is populated.
// ============================================================================

describe('QA finding A -- quote_verbatim never falls back to claim_text', () => {
  test('REGRESSION LOCK: evidence.quote_verbatim NULL + claims.text_verbatim set -> quote_verbatim: null, claim_text populated, quote_source: null', () => {
    const a = attr();
    const r = row({
      claim_id: 'c42',
      text_verbatim: 'Our own paraphrase of what this founder built.', // system-generated claim text
      evidence: [evid({ tier: 'documented', quote_verbatim: null, source_url: 'https://example.com/post' })], // no real quote
    });
    const resolved = resolvePositiveAttributeForCandidate(a, [r]);
    assert.equal(resolved.state, 'matched');
    const view = buildAttributeView(a, resolved, 1.0);
    assert.equal(view.evidence.quote_verbatim, null); // MUST be null -- never claims.text_verbatim
    assert.equal(view.evidence.claim_text, 'Our own paraphrase of what this founder built.');
    assert.equal(view.evidence.quote_source, null);
    assert.equal(view.evidence.source_url, 'https://example.com/post'); // source_url is independent of the quote and still travels
  });

  test('a REAL quote on the evidence entry populates quote_verbatim AND claim_text side by side, quote_source: "evidence"', () => {
    const a = attr();
    const r = row({
      claim_id: 'c43',
      text_verbatim: 'Our own paraphrase.',
      evidence: [evid({ tier: 'discovered', quote_verbatim: 'We shipped v1 to three pilot customers in March.', source_url: 'https://example.com/blog' })],
    });
    const resolved = resolvePositiveAttributeForCandidate(a, [r]);
    const view = buildAttributeView(a, resolved, 0.7);
    assert.equal(view.evidence.quote_verbatim, 'We shipped v1 to three pilot customers in March.');
    assert.equal(view.evidence.claim_text, 'Our own paraphrase.'); // both present, distinctly named -- never conflated
    assert.equal(view.evidence.quote_source, 'evidence');
  });

  test('neither a real quote nor claim text exists -> both null, quote_source null (never throws)', () => {
    const a = attr();
    const r = row({ claim_id: 'c44', evidence: [evid({ tier: 'inferred', quote_verbatim: null })] });
    const resolved = resolvePositiveAttributeForCandidate(a, [r]);
    const view = buildAttributeView(a, resolved, 0.4);
    assert.equal(view.evidence.quote_verbatim, null);
    assert.equal(view.evidence.claim_text, null);
    assert.equal(view.evidence.quote_source, null);
  });

  test('full score() round trip: a matched attribute with no real quote never presents claim_text as quote_verbatim', () => {
    const attributes = [attr({ id: 'p1' })];
    const fetchedRows = {
      p1: [row({
        founder_id: 'f1', topic: 'founder.expertise.vertical_tenure',
        text_verbatim: 'System-generated summary of this founder.',
        evidence: [evid({ tier: 'documented', quote_verbatim: null })],
      })],
      founders: [founder({ founder_id: 'f1' })],
    };
    const result = score(compiledPlan(attributes), fetchedRows);
    const item = result.items.find((it) => it.founder_id === 'f1');
    const attrView = item.attributes.find((a2) => a2.id === 'p1');
    assert.equal(attrView.evidence.quote_verbatim, null);
    assert.equal(attrView.evidence.claim_text, 'System-generated summary of this founder.');
  });
});

// ============================================================================
// QA gate, finding B -- structural (company-scoped) claim rows carry no
// `founder_id` (live: 49 `company.*` claims, exactly 1 with a founder_id)
// and must resolve through the candidate founder's `company_id`/
// `application_id` (via `api_founders`, §4.1) instead. The fetch/descriptor
// itself is UNCHANGED -- `lib/f10/plan.js` already filters by `topic` only,
// never by `founder_id`, so every company-scoped row was already being
// fetched; the defect was entirely in this module's in-memory founder
// resolution.
// ============================================================================

describe('QA finding B -- resolveRowFounderIds / buildFounderIndex', () => {
  test('a row WITH founder_id resolves to that founder ONLY, ignoring company/application entirely', () => {
    const idx = buildFounderIndex([
      founder({ founder_id: 'f1', company_id: 'co1' }),
      founder({ founder_id: 'f2', company_id: 'co1' }), // co-founder at the SAME company
    ]);
    const r = row({ founder_id: 'f1', company_id: 'co1' });
    assert.deepEqual(resolveRowFounderIds(r, idx), ['f1']); // never ['f1', 'f2'] -- a founder-specific claim must not bleed onto a co-founder
  });

  test('a row with NO founder_id resolves to every CURRENT founder of its company_id', () => {
    const idx = buildFounderIndex([
      founder({ founder_id: 'f1', company_id: 'co1' }),
      founder({ founder_id: 'f2', company_id: 'co1' }),
      founder({ founder_id: 'f3', company_id: 'co2' }),
    ]);
    const r = row({ founder_id: null, company_id: 'co1', topic: 'company.sector' });
    const resolved = resolveRowFounderIds(r, idx).slice().sort();
    assert.deepEqual(resolved, ['f1', 'f2']);
  });

  test('a row with NO founder_id and NO company_id match, but a matching application_id, resolves via application_id', () => {
    const idx = buildFounderIndex([founder({ founder_id: 'f1', company_id: null, application_id: 'app1' })]);
    const r = row({ founder_id: null, company_id: null, application_id: 'app1', topic: 'company.geography_country' });
    assert.deepEqual(resolveRowFounderIds(r, idx), ['f1']);
  });

  test('a row with no founder_id and no matching company/application resolves to nobody (not an error)', () => {
    const idx = buildFounderIndex([founder({ founder_id: 'f1', company_id: 'co1' })]);
    const r = row({ founder_id: null, company_id: 'co-unknown', application_id: null, topic: 'company.sector' });
    assert.deepEqual(resolveRowFounderIds(r, idx), []);
  });

  test('indexRowsByFounder places a company-scoped row under every current founder of that company', () => {
    const idx = buildFounderIndex([
      founder({ founder_id: 'f1', company_id: 'co1' }),
      founder({ founder_id: 'f2', company_id: 'co1' }),
    ]);
    const companyRow = row({ founder_id: null, company_id: 'co1', topic: 'company.sector', claim_id: 'cshared' });
    const grouped = indexRowsByFounder([companyRow], idx);
    assert.deepEqual(grouped.get('f1'), [companyRow]);
    assert.deepEqual(grouped.get('f2'), [companyRow]);
  });
});

describe('QA finding B -- full score() round trip: structural attributes now match through the company join', () => {
  test('a founder with NO row of their own still resolves company.sector via their company_id', () => {
    // Reproduces the live defect exactly: `api_claims` returns a
    // company.sector row for company 'co1' with founder_id NULL. f1 is the
    // current founder of co1 (per api_founders' own company_id column).
    const attributes = [
      attr({
        id: 'sector_ai_infra', kind: 'structural', op: 'eq', value: 'ai-infra', weight: WEIGHTS.structural,
        target: { type: 'claim_topic', value: 'company.sector' },
      }),
    ];
    const fetchedRows = {
      sector_ai_infra: [row({
        founder_id: null, company_id: 'co1', application_id: null,
        topic: 'company.sector', value: 'ai-infra',
        evidence: [evid({ tier: 'documented' })],
      })],
      founders: [founder({ founder_id: 'f1', company_id: 'co1' })],
    };
    const result = score(compiledPlan(attributes), fetchedRows);
    assert.equal(result.total, 1); // f1 IS gathered as a candidate -- pre-fix this was 0
    const item = result.items.find((it) => it.founder_id === 'f1');
    assert.ok(item, 'f1 must appear as a scored candidate');
    const attrView = item.attributes.find((a) => a.id === 'sector_ai_infra');
    assert.equal(attrView.state, 'matched');
    assert.equal(attrView.tier_credit, 1.0);
  });

  test('a founder with NO company still resolves unknown for a structural attribute, never mismatch (three-state semantics unchanged)', () => {
    const attributes = [
      attr({
        id: 'sector_ai_infra', kind: 'structural', op: 'eq', value: 'ai-infra', weight: WEIGHTS.structural,
        target: { type: 'claim_topic', value: 'company.sector' },
      }),
      attr({ id: 'p1' }), // a positive provenance attribute so f1 is gathered as a candidate at all
    ];
    const fetchedRows = {
      sector_ai_infra: [row({
        founder_id: null, company_id: 'co-someone-else', topic: 'company.sector', value: 'ai-infra',
        evidence: [evid({ tier: 'documented' })],
      })],
      p1: [row({ founder_id: 'f1', topic: 'founder.expertise.vertical_tenure', evidence: [evid({ tier: 'documented' })] })],
      founders: [founder({ founder_id: 'f1', company_id: null })], // f1 has NO company
    };
    const result = score(compiledPlan(attributes), fetchedRows);
    const item = result.items.concat(result.low_confidence).find((it) => it.founder_id === 'f1');
    const attrView = item.attributes.find((a) => a.id === 'sector_ai_infra');
    assert.equal(attrView.state, 'unknown'); // never 'mismatch' -- absence of a company is not evidence of anything
  });

  test('a company-scoped mismatch (contradicted claim, no founder_id) still resolves mismatch through the company join', () => {
    const attributes = [
      attr({
        id: 'geo_de', kind: 'structural', op: 'eq', value: 'DE', weight: WEIGHTS.structural,
        target: { type: 'claim_topic', value: 'company.geography_country' },
    ...
    const fetchedRows = {
      geo_de: [row({
        founder_id: null, company_id: 'co1', topic: 'company.geography_country', value: 'NL',
        evidence: [evid({ tier: 'discovered' })],
      })],
      p1: [row({ founder_id: 'f1', topic: 'founder.expertise.vertical_tenure', evidence: [evid({ tier: 'documented' })] })],
      founders: [founder({ founder_id: 'f1', company_id: 'co1' })],
    };
    const result = score(compiledPlan(attributes), fetchedRows);
    const item = result.items.find((it) => it.founder_id === 'f1');
    const attrView = item.attributes.find((a) => a.id === 'geo_de');
    assert.equal(attrView.state, 'mismatch'); // NL != DE, and the join must not weaken the existing mismatch rule
  });

  test('two co-founders at the same company both benefit from one shared company.sector claim, but a founder-specific claim never bleeds between them', () => {
    const attributes = [
      attr({
        id: 'sector_ai_infra', kind: 'structural', op: 'eq', value: 'ai-infra', weight: WEIGHTS.structural,
        target: { type: 'claim_topic', value: 'company.sector' },
      }),
      attr({ id: 'p1' }), // provenance, founder-scoped
    ];
    const fetchedRows = {
      sector_ai_infra: [row({
        founder_id: null, company_id: 'co1', topic: 'company.sector', value: 'ai-infra',
        evidence: [evid({ tier: 'documented' })],
      })],
      // Only f1 has a personal provenance claim -- f2 must NOT inherit it.
      p1: [row({ founder_id: 'f1', topic: 'founder.expertise.vertical_tenure', evidence: [evid({ tier: 'documented' })] })],
      founders: [founder({ founder_id: 'f1', company_id: 'co1' }), founder({ founder_id: 'f2', company_id: 'co1' })],
    };
    const result = score(compiledPlan(attributes), fetchedRows);
    const all = result.items.concat(result.low_confidence);
    const f1 = all.find((it) => it.founder_id === 'f1');
    const f2 = all.find((it) => it.founder_id === 'f2');

    assert.equal(f1.attributes.find((a) => a.id === 'sector_ai_infra').state, 'matched'); // shared company fact
    assert.equal(f2.attributes.find((a) => a.id === 'sector_ai_infra').state, 'matched'); // shared company fact

    assert.equal(f1.attributes.find((a) => a.id === 'p1').state, 'matched'); // f1's OWN claim
    assert.equal(f2.attributes.find((a) => a.id === 'p1').state, 'unknown'); // f2 does NOT inherit f1's personal claim
  });
});
