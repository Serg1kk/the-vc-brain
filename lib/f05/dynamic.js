...
//   5. THE ENTITY GATE RUNS ON SUPPORTS CANDIDATES TOO, not only
//      contradicts -- a POST-INCIDENT ruling, not the original design. This
//      branch's own first live run (against application 07f00002-0000-0000-
//      0000-000000000004, "GameLoop") measured the failure mode directly: a
//      plain company-name search surfaced `gameloop.com`, the real,
//      unrelated Android-emulator product, and an earlier (1-keyword)
//      version of passesRelevanceGate() let it through as `supports`,
//      flipping two real claims to `verified`. entity_gate.js's own header
//      scopes itself to contradiction candidates ("supports evidence carries
//      no false-accusation risk") -- correct for lib/f05/run.js's OTHER two
//      checks (their raw_signal FK comes from an independent identity-
//      resolution process), wrong for THIS branch's open web search, where
//      false CORROBORATION is at least as dangerous as false contradiction:
//      `verified` is the label an investor actually trusts. lib/f05/run.js
//      (task C3's own runFactualDynamicCheck) therefore calls
//      applyEntityGate() on every candidate this file hands it, regardless
//      of decideEvidenceAction()'s action -- this file's own passesRelevanceGate
//      is a cheap PRE-filter ahead of that, not a substitute for it.
// ============================================================================
//
// ============================================================================
...
// about the same subject", applied BEFORE a result is allowed to become a
// CANDIDATE at all -- i.e. before the entity gate (lib/f05/entity_gate.js's
// applyEntityGate, wired in by lib/f05/run.js for BOTH supports and
// contradicts candidates from this branch, post-incident ruling recorded at
// this file's own header) ever runs on it. Cheap first, expensive/precise
// second -- this gate rejects the bulk of same-named noise before a
// candidate is even worth the entity gate's per-candidate work.
//
...
// would -- see classifySourceKind/applyEntityGate for those). It is now a
// cheap PRE-filter, not the only line of defence: post-incident, the team
// lead ruled that lib/f05/run.js must also run applyEntityGate() on SUPPORTS
// candidates from this branch, not only on contradicts (entity_gate.js's own
// header scopes itself to contradiction candidates -- correct for the OTHER
// two checks in run.js, whose raw_signal FK comes from an independent
// identity-resolution process, but not for this branch's open web search,
// where false corroboration is at least as dangerous as false contradiction:
// `verified` is the label an investor actually trusts). This gate still runs
// FIRST and cheaply rejects the bulk of same-named noise before a candidate
// ever reaches the entity gate; a sufficiently generic claim (one
// contributing zero distinctive keywords) falls back to a name-only check --
// flagged here rather than silently overstated.
