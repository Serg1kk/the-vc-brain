...
describe('checkRobots returns the raw text so further urls need no refetch', () => {
  test('the same robots text gates EACH url, because rules are path-scoped', async () => {
    // QA re-check gap: only the seed url was gated, while /map hands up to 5
    // more urls to /extract. `Disallow: /blog` alongside an allowed `/` is an
    // ordinary robots.txt, so an origin-level check is not sufficient.
    const body = ['User-agent: *', 'Disallow: /blog', 'Disallow: /private'].join('\n');
    const f = stubFetch({ 'https://x.com/robots.txt': { status: 200, body } });

    const seed = await checkRobots('https://x.com/', f, UA);
    assert.equal(seed.allowed, true);
    assert.equal(typeof seed.text, 'string');
    assert.ok(seed.text.includes('Disallow: /blog'));

    // Mapped urls evaluated against the SAME text, no second fetch.
    assert.equal(isCrawlAllowed(seed.text, 'https://x.com/about', UA).allowed, true);
    assert.equal(isCrawlAllowed(seed.text, 'https://x.com/blog', UA).allowed, false);
    assert.equal(isCrawlAllowed(seed.text, 'https://x.com/blog/post-1', UA).allowed, false);
    assert.equal(isCrawlAllowed(seed.text, 'https://x.com/private/x', UA).allowed, false);
  });

  test('an origin-level-only check would have wrongly crawled the disallowed paths', () => {
    // Guards the regression directly: if someone "simplifies" back to gating
    // only the seed, this asserts the difference is real and not theoretical.
    const body = ['User-agent: *', 'Disallow: /blog'].join('\n');
    const seedAllowed = isCrawlAllowed(body, 'https://x.com/', UA).allowed;
    const mappedAllowed = isCrawlAllowed(body, 'https://x.com/blog', UA).allowed;
    assert.equal(seedAllowed, true);
    assert.equal(mappedAllowed, false);
    assert.notEqual(seedAllowed, mappedAllowed);
  });
});
