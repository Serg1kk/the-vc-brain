...
>
> ✅ **Общие DB-файлы: вопрос закрыт.** Они какое-то время были незакоммичены (DDL фичи 07
> переплетён с нашим, одна строка в `smoke.sql` действительно общая), затем потеряны при затирании
> рабочей копии в ~06:45, затем восстановлены из живой базы и закоммичены как `edee0df` сразу за
> фичи 02+03+07. Перепроверено после восстановления: свип `score_components` в `purge_founder` и
> `REVOKE TRUNCATE` на месте, `apply.sh` + `smoke.sql` зелёные, 77 тестов проходят, e2e не изменился.
...
- **AI-логика (ОБЯЗАТЕЛЬНО `ai-agent-builder`):** 4 агента-суб-скорера (execution-signals, expertise-signals, leadership-sales-proxies, red-flags) — ключевые промпты продукта.
- **n8n (ОБЯЗАТЕЛЬНО, два n8n-агента):** `score-founder`, `aggregate-score` (детерминированная Code-нода — формулой владеет бэкенд).
- **Дата-модель:** @database-engineer — scores append-only + config-строка формулы/весов; согласовать с 01.
- **UX/Дизайн:** не здесь (визуализация скора — в 09); но JSON-контракт score-breakdown определить ВМЕСТЕ с @designer, чтобы карточка могла его отрисовать.
- **QA:** @qa-engineer — тесты детерминизма формулы, missing→confidence (не score), независимость осей (REQ-002/003).
