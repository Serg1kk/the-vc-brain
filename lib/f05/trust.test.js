...
describe('isClaimInScope (design SS8.1)', () => {
  ...
  test('route 2 RESTRICTION (QA pass-1 finding 1, load-bearing): same company, but card_application_id ' +
    'points at a DIFFERENT (sibling) application -- excluded', () => {
    const row = claimRow({ card_application_id: 'app-sibling', card_company_id: COMPANY_A, card_founder_id: null });
    assert.equal(isClaimInScope(row, CTX), false);
  });

  // Note: "route 2: company card on the application's own company" above
  // already covers the legitimate card_application_id === null case; this
  // finding-1 restriction only needed a NEGATIVE case for a different
  // application id, which is the test right above.
