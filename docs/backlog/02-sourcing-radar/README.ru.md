...
Status: **ГОТОВО** (2026-07-19 · QA-гейт пройден · 265 тестов · см. [done.md](done.md)) · Зависит от: 01 (схема), 07 (thesis gate)

> **Дизайн — источник истины: [design.md](design.md)** (на английском). Он отменяет секцию
> «Implementation view» ниже, написанную до проверки материалов спонсора и живых API. Девять
> допущений этого README не пережили контакта с реальными данными; §2.1, §4.1 и §7.1 дизайна
> фиксируют каждое с замером. В частности: воронка — ~1380 кандидатов за 14-дневное окно
> (не 40–80), лишь ~36% постов Show HN ведут на GitHub (значит основной путь — продуктовый URL,
> а не репозиторий), владелец артефакта в 11% случаев организация, а Tavily `/map` возвращает
> ноль URL на реальных личных сайтах.
>
> Доказательство сквозной работы: фаундер, найденный радаром — он никуда не подавался и дека
> у него нет — получил настоящий Founder Score **60.76** (confidence 0.61) от пайплайна фичи 03,
> причём все 16 процитированных клеймов прослеживаются через evidence до 5 сырых наблюдений из
> 3 источников. См. [tracker.md](tracker.md).
