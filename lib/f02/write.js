...
function makeClient({ supabaseUrl, serviceRoleKey } = {}) {
  ...
  // patch -- PATCH one or more rows. Used ONLY for the Tier-1/Tier-2 company
  // rename (see the phase-invariant anchor below). Legal because `companies`
  // is a MUTABLE table: it carries updated_at and has no forbid_mutation
  // trigger, unlike raw_signals/evidence/scores/events where an UPDATE would
  // raise P0001.
  async function patch(table, filters, row) {
    const params = new URLSearchParams();
    for (const [k, v] of Object.entries(filters || {})) params.append(k, v);
    return restRequest('PATCH', `${table}?${params.toString()}`, { body: row, prefer: 'return=minimal' });
  }
  ...
  return {
    restRequest,
    selectOne,
    patch, insertAlways, insertIdempotent };
}
...
async function applyWriteSet(writeSet, opts = {}) {
  ...
  // constraint.
  //
  // ⚠️ PHASE-INVARIANT ANCHOR (QA re-check finding, proven live on three real
  // candidates: kaiT2en, brainwavesio, astrio-labs).
  //
  // The n8n workflow calls buildWriteSet TWICE for the same candidate: Tier 1
  // with `capabilities.github = false`, Tier 2 with full data. pipeline.js
  // derives `company.name` from the artifact owner's TYPE -- which Tier 1
  // cannot know. So for an Organization-owned artifact, Tier 1 names the
  // company after the REPO (`peek-cli`) and Tier 2 after the ORG
  // (`puffinsoft`) -- two different names, therefore two different rows, with
  // the founder's card pinned to the Tier-1 fabrication while the real
  // organisation sits orphaned. That is design §4.1 tier 3 violated in
  // production, and it happened on 3 of 3 org-owned candidates.
  //
  // The bug is that the company's identity was keyed on a name that DEPENDS ON
  // HOW MUCH DATA WE HAD. The fix is to key it on something identical in both
  // phases: the Show HN item id, which is what `applications` is already
  // deduplicated by. If an application for this hn_item_id already exists, its
  // company IS this candidate's company -- whatever either phase decided to
  // call it. Name divergence then becomes a rename (handled below), never a
  // split.
  let companyId;
  const anchorHnItemId = writeSet.application
    && writeSet.application.artifact_links
    && writeSet.application.artifact_links.hn_item_id;
  if (anchorHnItemId) {
    const anchorApp = await client.selectOne(
      'applications',
      { kind: `eq.${writeSet.application.kind}`, 'artifact_links->>hn_item_id': `eq.${anchorHnItemId}` },
      'id,company_id'
    );
    if (anchorApp && anchorApp.company_id) {
      companyId = anchorApp.company_id;
      created.company = false;
      // Tier 2 knows better than Tier 1: if it resolved a real organisation,
      // correct the name in place rather than minting a second row.
      // `companies` is mutable (it carries updated_at), so this is legal.
      if (writeSet.decisions && writeSet.decisions.orgIsCompany && writeSet.company.name) {
        const current = await client.selectOne('companies', { id: `eq.${companyId}` }, 'id,name');
        if (current && current.name !== writeSet.company.name) {
          await client.patch('companies', { id: `eq.${companyId}` }, { name: writeSet.company.name });
          warnings.push(`company renamed '${current.name}' -> '${writeSet.company.name}' (Tier 2 resolved the artifact owner as an Organization)`);
        }
      }
    }
  }
  if (companyId) {
    // anchored -- skip the name/domain lookups entirely
  } else if (writeSet.company.domain) {
