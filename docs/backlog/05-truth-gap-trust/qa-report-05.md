...
Two reproducible defects, both root-caused to specific lines of code, both capable of producing
exactly the failure mode this feature exists to prevent (a wrong or fabricated signal reaching an
investor). Finding 1 is not hypothetical — it is demonstrated against the live database with a real
`scores` row it produced. Finding 2 is dormant in the current corpus but is a structural property of
`quote_guard.js`, not a data fluke, and will fire on ordinary pitch-deck language the moment it does.
