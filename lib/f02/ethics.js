...
async function checkRobots(url, fetchFn, userAgent) {
  ...
  try {
    ...
    // `text` is returned so a caller can evaluate FURTHER urls from the same
    // origin without refetching -- robots rules are path-scoped, so every url
    // handed to a crawler needs its own check, not just the seed.
    return { allowed: verdict.allowed, checked: true, status, rule: verdict.rule, reason: verdict.reason, text };
  } catch (e) {
