...
async function runFactualDynamicCheck(params) {
  ...
  for (const c of candidates) {
    ...
    if (c.action === 'supports') {
      // design.md SS6: "Only CONTRADICTION candidates ever reach [the entity]
      // gate" -- a supports row carries no false-accusation risk and is
      // written directly, same as the other two checks' own 'clean'/no-
      // mismatch branches.
      const evidenceRow = await buildEvidenceRow({
        claimId: row.claim_id, relation: 'supports', tier: c.tier,
    ...
    // action === 'contradicts' -- entity gate MANDATORY (team lead
    // instruction, restating design SS6), steps 1-2 only (matchWithLlm
    // omitted -- owned by C1b).
    //
    // ⚠️ `rawSignal` is passed as `null` here, DELIBERATELY -- not an
    // oversight. entity_gate.js's step 1 resolves "by construction" when the
    // raw_signal's OWN founder_id/company_id FK matches the claim's entity.
    // For gh_provenance/quote_guard (the other two branches), that FK comes
    // from an INDEPENDENT identity-resolution process (ingest_commits.js's
    // checkable-founder resolution; feature 02/04's own writers) that ran
    // before this feature ever touched the row, so a step-1 match is
    // genuine evidence the CONTENT is about the right entity. HERE, this
    // very function is what just assigned founder_id/company_id to the
    // raw_signals row it created two paragraphs above -- required at
    // insert time for GDPR purge-scoping (design SS10.2), NOT because the
    // search result's CONTENT has been shown to be about that entity. If
    // that same self-assigned FK were also fed into the gate, step 1 would
    // resolve EVERY candidate trivially and unconditionally, defeating the
    // gate's entire purpose (the >80% false-contradiction rate design SS1.1
    // cites). Proven necessary by this task's own exploratory Tavily call,
    // 2026-07-19: a plain company-name search for the fictional "GameLoop"
    // returned `gameloop.com`, the real, unrelated Android-emulator product
    // -- exactly the wrong-entity match the gate exists to catch, and step 2
    // (registrable-domain match against `gameloop-thesis07.example`) is what
    // correctly fails it, PROVIDED step 1 is not given a free pass first.
    const entity = buildEntityForRow(row, entityContext);
    const gate = await applyEntityGate({
      claimId: row.claim_id,
    ...
    if (gate.resolved) {
      const evidenceRow = await buildEvidenceRow({
        claimId: row.claim_id, relation: 'contradicts', tier: c.tier,
        quoteVerbatim, sourceUrl: c.result.url || null, rawSignalId,
        checkId: 'tavily_dynamic', candidateKey,
      });
      evidenceRows.push(evidenceRow);
