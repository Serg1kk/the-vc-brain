...
> transcribed from the real files, not guessed. Three of the five lib files are marked `rework` in
> `tracker.md` (T5 `validate.js`, T6 `identity.js`, T7 `hashing.js`); the specific defects
> (`new URL()` in `identity.js`, no crypto polyfill guard in `hashing.js`) are called out at their
> exact wiring point below (§0.1, §3) rather than assumed fixed. This pass also folds in eleven
> corrections from an adversarial plan review (five urgent, six follow-up) and adds a fifth
> workflow, `f08-followup-create` (§10.3, plan.md T19), that the first pass missed entirely.
...
| `globalThis.crypto` polyfill — **required**, see §0.1 | 02's "Runtime discoveries"; confirmed independently by `hashing.js`'s own header comment | Prepend the guard below to every Code node that calls `hashing.js`'s `sha256Hex`/`hashFields`/`contentHash.*` |
...
### 0.1 The crypto polyfill — required; the URL polyfill is deliberately NOT used

**Only `globalThis.crypto` needs a runtime patch in this feature — unlike 02, `lib/f08/*.js` does
not use `new URL()` at all**, by an explicit team-lead correction to `plan.md` T5 ("`URL` is
undefined in the n8n Code sandbox... reimplement without `URL` rather than lean on a Code-node
polyfill block"). `lib/f08/validate.js`'s own `parseAbsoluteHttpUrl` is a hand-written regex/string
parser with **no `try/catch` and no `URL` reference anywhere** (its header comment explains why: 02
found live that `new URL()`'s `ReferenceError` was silently swallowed by an existing `try/catch`
written for genuinely-unparseable input, degrading into a wrong-but-plausible answer with nothing
in the logs). `lib/f08/identity.js` still has one `new URL(raw)` call at the time of writing
(`tracker.md` T6, status `rework`, "same `new URL()` issue") — **the fix in flight is to replace
it with the same regex parser, not to add a polyfill**, so this spec does not wire a URL polyfill
for it either. **Verify this landed before building the generator** — `grep -n "new URL" lib/f08/*.js`
should return nothing once T5/T6 close.

`hashing.js`, by contrast, **must** call the real Web Crypto API (there is no way to compute
SHA-256 by string manipulation), and its own header confirms `globalThis.crypto.subtle` is
unavailable in this sandbox without help. Prepend this exact guard (02's `RUNTIME_POLYFILL_JS`,
narrowed to only the piece 08 needs) to the top of **every** Code node that calls into
`hashing.js`:

```js
if (typeof globalThis.crypto === 'undefined' || typeof globalThis.crypto.subtle === 'undefined') {
  globalThis.crypto = require('crypto').webcrypto;   // bare 'crypto', not 'node:crypto' — the
}                                                      // require-resolver does an exact string
                                                       // match against NODE_FUNCTION_ALLOW_BUILTIN
```

Every `hashing.js` function is `async` (`sha256Hex`, `hashFields`, and all three `contentHash.*`
recipes) — every Code node that calls one must itself run in async/awaited style, matching how
`hashing.js`'s own header already flags this.
...
unlike `crypto`/`URL`/`URLSearchParams`, which needed the `require()`-based polyfill precisely
*because* the JS Task Runner sandbox strips certain globals — see 02's README). **Verify live
before trusting this** — if `Buffer` is missing too, the same `typeof`-guarded polyfill pattern
applies (`globalThis.Buffer = require('buffer').Buffer` — `'buffer'` is not currently on the
`NODE_FUNCTION_ALLOW_BUILTIN` allow-list in `infra/n8n/docker-compose.yml`, so this would need a
devops addition to that list, flagged here rather than discovered mid-build).
...
### 0.3.1 Catch-all error branch — mandatory on every one of the four workflows
...
(`Handle unexpected error` → `Respond: internal error`) to each of the four node tables below,
...
the four workflows, fetch `GET /api/v1/executions/{id}?includeData=true` and confirm every
...
| `hashing.js` | `sha256Hex(text)`, `hashFields(...parts)`, `contentHash.rawSignal({application_id,source,content_key})`, `contentHash.claim({application_id,card_id,topic,item_key})`, `contentHash.evidence({application_id,claim_id,relation,raw_signal_id})` — **all async**, all require the §0.1 crypto polyfill prepended | Every workflow, everywhere a `content_hash` is written — always salted with `application_id` per the file's own header rationale |
...
25 nodes (parallel-branch count depends on whether the Merge-optimization in step 16 is taken —
see the note there), plus the shared catch-all pair from §0.3.1. Full step sequence:
...
| 14 | `Write founder claims + evidence` | code (`hashing.js`) | From deck-claims-extractor output, §5.1 |
| 15 | `Update founders.full_name` | code | Only if `is_new_founder && founder_identity` (§3 note) |
...
| 23 | `Build IntakeResponse` | code | Assembles the exact §4.1 shape, `deck.pages`/`chars_extracted` = **0, never null**, on the `none` branch (§5) |
...
`applications.artifact_links._f08_deck_meta`, written once at step 24. **Everything else in the
...
`extra_files_stored` via `artifact_links.founder_links` length), which is more correct than a
...
```
claims: {
  card_id: <the 'founder' card from step 14>, topic, text_verbatim, value,
  ...
  content_hash: sha256('f08:claim:' + application_id + ':' + topic + ':' + quote_verbatim)  -- salted
...
evidence: {
  ...
  quote_verbatim, source_url: null, raw_signal_id: <the step-15 raw_signals row's id>,
  content_hash: sha256('f08:evidence:' + application_id + ':' + topic + ':' + quote_verbatim)
}
```
...
evidence: { claim_id, relation:'context', tier:'missing', quote_verbatim: null, source_url: null,
            raw_signal_id: <the step-15 raw_signals row>,
            content_hash: sha256('f08:evidence:missing:' + application_id + ':' + topic) }
            -- evidence.content_hash is NOT NULL in the schema even here (unlike claims.content_hash,
            which is nullable) -- there is no quote/source_url to hash from a missing marker, so the
            recipe above (topic + application_id only) is this spec's answer to a gap `hashing.js`
            must fill that neither `lib/f07/hashes.js` nor design.md spells out for the 08 case.
```
...
## 5. Calling `f07-thesis-gate` (step 17's Merge branch, or a plain sequential call)
...
raw_signals: { source:'interview_answer', payload:{criterion_id, question, answer_text},
  ...
  content_hash: sha256('f08:raw:'+application_id+':'+criterion_id+':answer') }
claims: { card_id, topic: <criterion_id mapped back to its topic, same map as §4.1's schema>,
  ...
  value: null, axis: null, source_kind:'interview', base_confidence: 0.30 (proposed — matches
    the `credit.self_asserted` anchor already in this exact `score_formulas` config, §5.6 of
    03/design.md's own table; no other "low base confidence for an unverified self-report" number
    exists anywhere else in this schema to borrow instead), verification_status:'unverified',
  content_hash: sha256('f08:claim:'+application_id+':'+topic+':interview') }
evidence: { claim_id, relation:'supports', tier:'discovered' (signal_sources.interview_answer.
  ...
  content_hash: sha256('f08:evidence:'+application_id+':'+topic+':interview') }
```
...
| 3 | `Hash token, look up interview` | code (`hashing.js`) | `sha256(token)`; `GET interviews?kind=eq.follow_up&share_token=eq.<hash>&select=*&order=created_at.asc&limit=1`. **Read-only — never touches `status`** |
...
32 random bytes via `crypto.getRandomValues(new Uint8Array(32))` (Web Crypto, same polyfill guard
as §0.1 covers — `getRandomValues` is part of the same `globalThis.crypto` surface), hex-encoded
for the **raw** token returned to the caller; **only `sha256Hex(raw_token)` is stored** in
...
7. **`evidence.content_hash` recipe for a `missing` marker** (`sha256('f08:evidence:missing:' +
   application_id + ':' + topic)`) — required because the column is `NOT NULL` even though
   `claims.content_hash` is nullable for the same row, and `hashing.js`'s own `contentHash.evidence`
   recipe (`application_id::claim_id::relation::raw_signal_id`) already covers this case generically
   (a missing-marker's `raw_signal_id` still points at the step-11 `raw_signals` row) — **on
   inspection this assumption may already be redundant with the real `hashing.js` export**; verify
   `contentHash.evidence({application_id, claim_id, relation:'context', raw_signal_id})` produces a
   collision-free hash for this case before adding a bespoke recipe.
...
12. **`Buffer` is assumed to be an available Code-node-sandbox global** for the Storage upload
    (§0.2) — unlike `crypto`, which needed a polyfill (the URL polyfill turned out to be
    unnecessary, §0.1). Flagged for live verification; if wrong, `'buffer'` needs adding to
